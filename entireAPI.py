...
def get_line_data(cursor):
    ...
    for row in raw_lines:
        lines_data.append({
            ...
            "type": row.get('type'),
            "code": row.get('code'),
            # FIX: Reverted to use '/' to match the user's database column names.
            "colour": [
                row.get('colour/0'),
                row.get('colour/1'),
                row.get('colour/2')
...
def get_route_data(cursor):
    """Fetches all data from the 'routes' table."""
    cursor.execute("SELECT * FROM `routes`")
    return cursor.fetchall()

def get_map_data(cursor):
    """Fetches and transforms all data from the 'mapRoutes' table."""
